...
import com.resourceradar.repository.EmployeeAllocationRepository;
...
public class EmployeeAllocationServiceImpl implements EmployeeAllocationService {
	...
	@Override
	public EmployeeAllocation updateEmployeeAllocation(EmployeeAllocation employeeAllocation)
			throws EmployeeAllocationNotFoundException {
		EmployeeAllocation existingEmployeeAllocation = employeeAllocationRepository
				.findById(employeeAllocation.getId()).orElse(null);

		if (existingEmployeeAllocation == null) {
			throw new EmployeeAllocationNotFoundException(
					"Employee allocation not found with id " + employeeAllocation.getId());
		}
